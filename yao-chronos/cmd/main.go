...
const (
	...
	DefaultConfigPath = "configs/chronos.yaml"
...
func main() {
	...
	// srv, err := server.NewChronosServer(cfg)
	// if err != nil {
	...
	go func() {
		...
		//     log.Fatalf("Failed to start chronos server: %v", err)
		// }
	...
	//     log.Fatalf("Failed to stop chronos server gracefully: %v", err)
	// }
...
func printHelp() {
	log.Printf(`%s %s - Centralized Transaction Sequencer for YaoVerse
...
Environment Variables:
  YAO_CHRONOS_CONFIG    Configuration file path (overrides -config flag)
  YAO_CHRONOS_LOG_LEVEL Log level (debug, info, warn, error)

Examples:
  %s                            # Use default configuration
  %s -config /etc/chronos.yaml  # Use custom configuration file
